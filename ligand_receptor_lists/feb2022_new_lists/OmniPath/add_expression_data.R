...
# this include manual correction of 47 NA's
# plus some additional comments
gene_list <- read.csv("ligand_receptor_lists/feb2022_new_lists/OmniPath/data/ligands_annot_manual.tsv", 
                       sep = "\t") %>% 
  # remove empty t2d genetic associations column
  select(-18) %>% 
  # remove the 8 genes which still have NA for gene symbol & ensembl
  drop_na(hgnc_symbol)
...
gene_list_reduced <- gene_list %>% 
  ...
  summarise(description = paste0(unique(description), collapse=", "),
            ...
            receptor_uniprot = paste0(unique(receptor_uniprot), collapse=", "),
            receptor_symbol = paste0(unique(receptor_symbol), collapse=", "),
            receptor_references = paste0(unique(receptor_references), collapse=", "),            
...
write_tsv(genes_expression, 
          "ligand_receptor_lists/feb2022_new_lists/OmniPath/data/ligands_annot_expr.tsv")
