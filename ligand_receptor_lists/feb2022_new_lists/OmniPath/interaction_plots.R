...
dat <- read.table("ligrec_interactions_filtered_noduplicates_final.tsv",
                  header = T) %>% 
...
ligands <- filter(dat, type == "ligands") %>% 
  group_by(gene_symbol) %>% 
  slice(1)

recept <- filter(dat, type == "receptors") %>% 
  group_by(gene_symbol) %>% 
  slice(1)
...
ggplot(dat, aes(x = type, y = gene_symbol)) +
  ...
  geom_text_repel(data = labels_recept, aes(label = gene_symbol),
                  nudge_x = 0.07, size = 3.5, hjust = 0,
                  direction = "y", box.padding = 0.05,
                  segment.size = 0.25) +
  geom_text_repel(data = labels_ligands, aes(label = gene_symbol),
                  nudge_x = -0.07, size = 3.5, hjust = 1,
                  direction = "y", box.padding = 0.05,
                  segment.size = 0.25) +
  scale_color_fermenter(palette = "YlGnBu",
                        direction = 1,
...
ggsave("interact_plot_labels.png", width = 5, height = 10)
...
# for labels:
dat_20res_source <- filter(dat_20res, type == "source") %>% 
  group_by(gene_symbol) %>% 
  slice(1)
dat_20res_target <- filter(dat_20res, type == "target") %>% 
  group_by(gene_symbol) %>% 
  slice(1)

# Plot
ggplot(dat_20res, aes(x = type, y = gene_symbol)) +
  geom_point(aes(size = n_interactions,
                 colour = n_resources),
             alpha = 0.7) +
  geom_text(data = dat_20res_source,
            aes(label = gene_symbol),
            nudge_x = -0.05, size = 1.4,
            hjust = 1) +
  geom_text(data = dat_20res_target,
            aes(label = gene_symbol),
            nudge_x = 0.05, size = 1.4,
            hjust = 0) +
  geom_line(aes(group = interaction_symbol,
